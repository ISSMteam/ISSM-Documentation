...
$$
\begin{cases}y_{t} = \overline{y}_{t}+\epsilon_{y,t} \\\epsilon_{y,t} \sim N\left(0,\sigma^{2}_{y}\right)\end{cases}
$$
...
- If timestep is a stochastic timestep: draw new stochastic perturbations,
  $$\boldsymbol{\epsilon}_{t} \sim N(\boldsymbol{0},\Sigma)$$
- If timestep is not a stochastic timestep: keep $$\boldsymbol{\epsilon}_{t}$$ unchanged.
